 ...
 *
 * `POST /schedules/:staffId/rules` is `requireAdmin` — when a studio opens is
 * the studio's decision. `POST /schedules/:staffId/overrides` is
 * `requireAdminOrSelf`, because an instructor taking next Tuesday off is
 * theirs. Collapsing both into one flag would either hand instructors a form
 * that 403s or hide the one thing they are entitled to do.
 */
    ...
    <section className="card schedule">
      ...
      {rules && working.length === 0 && (
        <div className="alert warn" role="status">
          ...
          them, however many classes they are qualified to teach.{' '}
          {canEditHours
            ? 'Add a pattern below.'
            : 'Only an owner or admin can set these, so ask the studio to add your hours.'}
        </div>
      )}
